...
int main(void)
{
  ...
  s = "  -3208.15E-13";    // para ver cómo trabaja atof
  x = atof( s );
  printf( "atof: cadena ASCII: %-17s double: %e\n", s, x );
  s = "8.7195642337X120";  // para ver cómo trabaja atof
  x = atof( s );
  printf( "atof: cadena ASCII: %-17s double: %e\n", s, x );
  s = "  -8995 libros";    // para ver cómo trabaja atoi
  i = atoi( s );
  printf( "atoi: cadena ASCII: %-17s int  : %d\n", s, i );
  s = "89954 pesetas";     // para ver cómo trabaja atol
  l = atol( s );
