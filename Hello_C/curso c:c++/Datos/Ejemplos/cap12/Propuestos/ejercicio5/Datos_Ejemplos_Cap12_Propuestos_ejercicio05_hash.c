...
// Iniciar el archivo índice y abrir el de datos
iniciarArchivos(tMatrizHash *mh)
{
  ...
  printf("Nombre del archivo índice: ");
  gets(nomFich);
  ...
  if (resp == 's')
  {
    ...
    if (!mh->pfInd) error("No se puede abrir el archivo índice");

    printf("Número de registros para el archivo índice: ");
    if ((scanf("%d", &n_regs) == 0) || n_regs < 1) n_regs = 101;
    // Crear un archivo índice con n_regs registros inicialmente
    ...
    if (ferror(mh->pfInd)) error("No se puede crear el archivo índice");
    strcpy(aux, nomFich); strcat(aux, ".dat");
    mh->pfDat = fopen(aux, "w+b");
    if (!mh->pfDat) error("No se puede abrir el archivo de datos");
    printf("Archivos índice y datos creados\n");
  }
  else
  {
    printf("Se abre el archivo existente\n");
...
int hashIn(tRegInd x, tMatrizHash mh)
{
  ...
  if (conta == mh.n_regs)
  {
    printf("error: archivo índice lleno\n");
    insertado = 0;
  }
  return insertado;
}
