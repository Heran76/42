...
//////////////////////////////////////////////////////////////////
// Ordenar un archivo utilizando el método de mezcla natural.
// Se trata de un archivo de texto que almacena una lista de
// nombres.
// El nombre del archivo se recibe a través de la línea de órdenes.
...
int mezclar(FILE *fa, FILE *fb, FILE *f)
{
  ...
  // archivos. La fusión se realiza entre pares de tramos
  ...
  while (!feof(fa) && !feof(fb))
  {
    if (strcmp(stra, strb) < 0)        // 1
    {
      ...
      {
        // Copiamos la cadena leída del archivo A
        strcpy(stra_ant, stra);
        fputs(stra, f);
        fgets(stra, MAX, fa);
      }
    ...
    else // 1
    {
      ...
      {
        // Copiamos la cadena leída del archivo B.
        strcpy(strb_ant, strb);
        fputs(strb, f);
        fgets(strb, MAX, fb);
      }
