/* Actualizar el archivo de notas
 * ejercicio03.c
 */
...
int main(int argc, char *argv [])
{
  ...
  if (argc != 3)
  { // Se toma por defecto los archivos 'alumnos' y 'modifi'
    argv[1] = "notas";
    argv[2] = "modifi";
  }
...
{ 
  ...
  while (1)
  {
    ...
    if (( i = strncmp(regA.nombre, regB.nombre, sizeof(regA.nombre)) ) == 0) 
    {
      ...
      n++;
      // Leer de nuevo de ambos archivos
      if (fread(&regB, bytesreg, 1, pfb) < 1) 
        regB.nombre[0] = 255;
      if (fread(&regA, bytesreg, 1, pfa) < 1)
        regA.nombre[0] = 255;
  ...
  // Copiar en el archivo notas todos los registros del temporal.
  // Para ello, creamos de nuevo el archivo notas.
  fclose(pfa);
  ...
  rewind(pfc); // volver al principio del archivo temporal
  for ( i = 0; i < n; i++ )
  {
