  ...
  while (1)
  {
    ...
    switch (op)
    {
      case 1: // se autoriza o no la reparación
        printf("\nNúmero de serie: ");
        leerCadena(numSerie, sizeof(numSerie));
      ...
      case 2: // buscar la posición en la matriz de un cliente
        printf("\nCódigo del cliente:  ");
        leerCadena(codigo, sizeof(codigo));
...
void DatosContrato(tcliente *pcliente, int i, int pos)
{
  ...
  while (getchar() != '\n');
  leerCadena(pcliente[i].ContratoCli.Contrato[pos].Descripcion,
             sizeof(pcliente[i].ContratoCli.Contrato[pos].Descripcion));

  printf("\nNúmero de serie:  ");
  leerCadena(pcliente[i].ContratoCli.Contrato[pos].NumSerie,
             sizeof(pcliente[i].ContratoCli.Contrato[pos].NumSerie));

  printf("\nNúmero del contrato:  ");
  leerCadena(pcliente[i].ContratoCli.Contrato[pos].NumContrato,
             sizeof(pcliente[i].ContratoCli.Contrato[pos].NumContrato));
...
void DatosCliente(tcliente *pcliente, int numClientesAct, int numCientesFinal)
{
  ...
  for (i = numClientesAct; i < numCientesFinal; i++)
  {
    ...
    printf("\nNombre: ");
    while (getchar() != '\n');
    leerCadena(pcliente[i].Nombre, sizeof(pcliente[i].Nombre));

    printf("\nApellidos:  ");
    leerCadena(pcliente[i].Apellidos, sizeof(pcliente[i].Apellidos));

    printf("\nDirección:  ");
    leerCadena(pcliente[i].Direccion, sizeof(pcliente[i].Direccion));

    printf("\nCódigo:  ");
    leerCadena(pcliente[i].Codigo, sizeof(pcliente[i].Codigo));
