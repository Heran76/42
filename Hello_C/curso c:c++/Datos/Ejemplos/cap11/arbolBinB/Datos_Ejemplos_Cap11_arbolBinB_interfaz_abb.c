...
// comparar, copiar, procesar y liberarmem.
//
...
int insertar(void *datos, Nodo **raiz)
{
  ...
  if ( actual == NULL ) // no se encontró el nodo, añadirlo
  {
    ...
    // este está vacío, del subárbol izquierdo de "último" si la
...
void *borrar(void *datos, Nodo **raiz)
{
  ...
  if ( actual != NULL ) // se encontró el nodo
  {
    ...
    else  // nodo con subárbol izquierdo y derecho
    {
      ...
      copiar(marcado->datos, actual->datos);
      if (actual->derecho == NULL) marcado->derecho = NULL;
      // Identificar el nuevo nodo a borrar
      marcado = actual; // este es ahora el nodo a borrar
      ultimo = sucesor;
      sucesor = actual->derecho;
    }
