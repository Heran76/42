...
void error1(tllseo *poliA, tllseo *poliB, tllseo *poliR);
...
Termino *leerTermino()
{
  ...
  scanf("%d", &t->exponenteDeY); while (getchar() != '\n');
...
void error1(tllseo *poliA, tllseo *poliB, tllseo *poliR)
{
  liberarMemoria(poliA);
  liberarMemoria(poliB);
  liberarMemoria(poliR);
  error();
}
...
int main(void)
{
  ...
  {
    termino = leerTermino();
    if (!termino) error1(&polinomioA, &polinomioB, &polinomioR);
    if (buscar(termino, &polinomioA))
    {
      ...
      if (!anyadir(termino, &polinomioA))
      {
        free(termino);
        error1(&polinomioA, &polinomioB, &polinomioR);
      }
  ...
  while (resp == 's')
  {
    termino = leerTermino();
    if (!termino) error1(&polinomioA, &polinomioB, &polinomioR);
    if (buscar(termino, &polinomioB))
    {
      aux = obtenerActual(&polinomioB);
      aux->coeficiente += termino->coeficiente;
      free(termino);
    }
    else
      if (!anyadir(termino, &polinomioB))
      {
        free(termino);
        error1(&polinomioA, &polinomioB, &polinomioR);
      }
